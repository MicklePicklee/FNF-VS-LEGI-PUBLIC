...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	{
		title = 'HUD Settings';
		rpcTitle = 'HUD Settings Menu'; //for Discord Rich Presence
		...
		if(noteSkins.length > 0)
		{
			if(!noteSkins.contains(ClientPrefs.data.noteSkin))
				ClientPrefs.data.noteSkin = ClientPrefs.defaultData.noteSkin; // we are fnaf
		...
		if(noteSplashes.length > 0)
		{
			if(!noteSplashes.contains(ClientPrefs.data.splashSkin))
				ClientPrefs.data.splashSkin = ClientPrefs.defaultData.noteSkin; // this is fucking vs legi
