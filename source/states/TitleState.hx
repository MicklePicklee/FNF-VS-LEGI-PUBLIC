...
class TitleState extends MusicBeatState
{
	...
	var titleTextColor:FlxColor = 0xFF3333CC;
	...
	function startIntro()
	{
		...
		Conductor.bpm = ClientPrefs.data.mainBPM;
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('modTeam-logo'));
		ngSpr.visible = false;
	...
	// JSON data
	var characterImage:String = 'legiTitle';
	var animationName:String = 'LEGI Dancing Beat instance 100';
	...
	public static var musicBPM:Float = ClientPrefs.data.mainBPM;
	...
	{
		if(Paths.fileExists('images/legiTitle.json', TEXT))
		{
			var titleRaw:String = Paths.getTextFromFile('images/legiTitle.json');
		...
		if(!closedState)
		{
			...
			switch (sickBeats)
			{
				...
				case 15:
					addMoreText('VS');
				case 16:
					addMoreText('LEGI'); // credTextShit.text += '\nFunkin';
			...
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					default: //Go back to normal ugly ass boring GF
						...
						FlxG.sound.playMusic(Paths.music(Paths.music(Paths.formatToSongPath(ClientPrefs.data.menuTheme))), 0);
						if (ClientPrefs.data.menuTheme == 'None')
							FlxG.sound.music.volume = 0;
						else
							FlxG.sound.music.fadeIn(4, 0, 0.7);
