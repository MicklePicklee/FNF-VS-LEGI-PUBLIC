...
class PlayState extends MusicBeatState
{
	...
	public function endSong()
	{
		...
		var diffName = Difficulty.getString().toLowerCase();
		var songNoMiss:String = Song.loadedSongName + '_nomiss_' + diffName;
		var weekNoMiss:String = WeekData.getWeekFileName() + '_nomiss_' + diffName;
		checkForAchievement([songNoMiss, weekNoMiss, 'welcome', 'predictable-outcome']);
		#end
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					// Set flag to unhide achievements on next menu open
					FlxG.save.data.unhideAchievementsOnMenu = true;
					FlxG.save.flush();
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in 0...separatedScore.length)
		{
			...
			numScore.x = placement + (25 * daLoop) - 90 + ClientPrefs.data.comboOffset[2];
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null)
	{
		...
		for (name in achievesToCheck) {
			...
			var diffName = Difficulty.getString().toLowerCase();
			var songNoMiss = Song.loadedSongName + '_nomiss_' + diffName;
			var weekNoMiss = WeekData.getWeekFileName() + '_nomiss_' + diffName;
			if (name != songNoMiss && name != weekNoMiss) // common achievements
            {
                switch(name)
                {
                    case 'welcome':
                        unlock = true;
					case 'predictable-outcome': // in the context of vs legi, the only playable week currently is week_legi - but this is just precautionary
						unlock = (WeekData.getWeekFileName() == 'week_legi') && (isStoryMode && storyPlaylist.length <= 1) && !usedPractice; // i could make it so you can't change difficulty or use practice - but the challenge is to beat the week itself, rather than beat the week under a certain set of circumstances
                }
            }
			else // any FC achievements, name should be "songName_nomiss_difficulty", e.g: "bopeebo_nomiss_hard";
			{
				if(songMisses < 1 && !changedDifficulty && !usedPractice || 
					isStoryMode && campaignMisses + songMisses < 1 && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
					unlock = true;
			}
