...
class Paths
{
	...
	inline static public function inst(song:String, ?modsAllowed:Bool = true, ?freeplayBodge:Bool = false):Sound
	{
		var diffName:String = Difficulty.getString(false).toLowerCase();
		var donSuffix:String = (diffName == "double or nothing" || diffName == "double-or-nothing") ? "-double-or-nothing" : "";
		if (freeplayBodge) donSuffix = ""; // mm mm m i love the smell of bodging at 4pm :D (thank you autofill - ok there was no need to suck your own cock off)

		return returnSound('${formatToSongPath(song)}/Inst' + donSuffix, 'songs', modsAllowed);
	}

	inline static public function voices(song:String, postfix:String = null, ?modsAllowed:Bool = true, ?freeplayBodge:Bool = false):Sound
	{
		var diffName:String = Difficulty.getString(false).toLowerCase();
		var donSuffix:String = (diffName == "double or nothing" || diffName == "double-or-nothing") ? "-double-or-nothing" : "";
		if (freeplayBodge) donSuffix = ""; // i am a bodger and this is my bodge :D (thank you autofill)

		var songKey:String = '${formatToSongPath(song)}/Voices';
		if(postfix != null) songKey += '-' + postfix;
		return returnSound(songKey + donSuffix, 'songs', modsAllowed, false);
	}
	...
	public static function loadAnimateAtlas(spr:FlxAnimate, path:String)
	{
		try
		{
			spr.frames = FlxAnimateFrames.fromAnimate(Paths.getPath('images/' + path));
