...
class Achievements {
	...
	{
		createAchievement('welcome',		{name: "Welcome to VS LEGI!", description: "Open the game for the first time."});
		createAchievement('another-beat', 	{name: "Another Beat?", description: "Change the Main Menu Theme."});
	...
	public static function load():Void
	{
		...
		if (FlxG.save.data != null && FlxG.save.data.unhiddenAchievements != null)
		{
			for (key in cast(FlxG.save.data.unhiddenAchievements, Array<Dynamic>))
			{
				var achieve = achievements.get(key);
				if (achieve != null) achieve.hidden = false;
			}
		}
	}
