...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		// Unhide all hidden achievements if the flag is set, BEFORE building the list
		if (FlxG.save.data.unhideAchievementsOnMenu == true)
		{
			if (FlxG.save.data.unhiddenAchievements == null)
				FlxG.save.data.unhiddenAchievements = [];

			for (key in Achievements.achievements.keys())
			{
				var achieve = Achievements.achievements.get(key);
				if (achieve != null && achieve.hidden == true)
				{
					achieve.hidden = false;
					if (!FlxG.save.data.unhiddenAchievements.contains(key))
						FlxG.save.data.unhiddenAchievements.push(key);
				}
			}
			FlxG.save.data.unhideAchievementsOnMenu = false;
			FlxG.save.flush();
		}
		...
		var menuBG = new FlxSprite().loadGraphic(Paths.image('backGround'));
